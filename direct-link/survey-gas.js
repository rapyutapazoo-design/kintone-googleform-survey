// ============================================================
// アンケート自動化 常設GAS（直リンク版）
// ============================================================
// 【中継版との違い】
//   QRの飛び先をGAS中継ページ(doGet)ではなく「Googleフォーム直リンク」にする版。
//   → Apps Script Webアプリ起因の「警告バナー」「マルチログインで開けない」問題を回避。
//   → QR導線の回答前ブロックは行わず、重複は集計時の重複排除で担保する。
//   doGet（中継ページ）は撤去。ウェブアプリとしてのデプロイは不要。
//   setupForm が QR用直リンク(form_qr_url・経路=QR事前入力)を生成しKintoneへ保存する。
//
// 【構成】
...
//   ※ Kintone導線は回答済みブロックあり。QR導線は集計時の重複排除で担保。
//
...
//
// 【デプロイ】ウェブアプリのデプロイは不要（時間トリガーのみ）。Webhookも使用しない。
// ============================================================
...
function setupForm(config, formId, formName) {
  ...
  // --- Kintone導線用の事前入力URL（部屋番号・氏名・経路・メールをplaceholderで残す） ---
  var prefillUrl = form.createResponse()
    .withItemResponse(roomItem.asTextItem().createResponse(ROOM_PLACEHOLDER))
    .withItemResponse(nameItem.asTextItem().createResponse(NAME_PLACEHOLDER))
    .withItemResponse(routeItem.asTextItem().createResponse(ROUTE_PLACEHOLDER))
    .withItemResponse(emailItem.asTextItem().createResponse(EMAIL_PLACEHOLDER))
    .toPrefilledUrl();
  var entryId = extractEntryId(prefillUrl);

  // --- QR導線用の直リンクURL（経路=QRのみ事前入力。部屋番号・氏名は住民が入力） ---
  // 警告バナー・マルチログイン問題を避けるため、QRはこのフォーム直リンクを指す。
  var qrUrl = form.createResponse()
    .withItemResponse(routeItem.asTextItem().createResponse('QR'))
    .toPrefilledUrl();
